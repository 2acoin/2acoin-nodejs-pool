...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height) {
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d for block version: %s', [result.height, result.difficulty, buffer[0], buffer[1]]);
...
function processShare(miner, job, blockTemplate, params){
	...
	// Specifically prepare for the 2ACoin hard fork
        if (hard_fork_version <= 4 && daemonType === "2acoin" ){
            log('info', logSystem, 'Mining pool algorithm: cryptonightlite variant 1, Hard fork version: v%d.%d', [shareBuffer[0], shareBuffer[1]]);
            ...
            log('info', logSystem, 'Mining pool algorithm: %s variant %d, Hard fork version: v%d.%d', [cnAlgorithm, cnVariant, shareBuffer[0], shareBuffer[1]]);
