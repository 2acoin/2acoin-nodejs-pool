    ...
    try {
        ...
        let options = {
            ...
            'headers' : {
                        ...
                        'x-cg-demo-api-key': 'CG-4caddJJfvNUcY4a2b5LysV21'
                     }
        }
        options['json'] = data
...
/**
 * Send RPC via wallet-api
**/
function walletAPI(host, port, path, data, callback){
    try {
        callback = callback || function(){};
        let options = {
            'uri': `${(port === 443 ? 'https' : 'http')}://${host}:${port}${path}`,
    ...
    return {
        ...
        rpcWallet: function(method, params, callback){
            if (config.wallet.api) {
                walletAPI(walletConfig.host, walletConfig.port, method, params, callback);
            } else {
