...
var cnHashing = require('2acoin-multi-hashing');
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height) {
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d for block version: v%d.%d', [result.height, result.difficulty, buffer[0], buffer[1]]);
...
Miner.prototype = {
    ...
    setNewDiff: function(newDiff){
        ...
        log('info', logSystem, 'Retargetting difficulty %d to %d for %s~%s', [this.difficulty, newDiff, this.login, this.workerName]);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s~%s@%s', [shareType, job.difficulty, shareDiff, miner.login, miner.workerName, miner.ip]);
}
...
function processShare(miner, job, blockTemplate, params){
	...
	// Specifically prepare for the 2ACoin hard fork
        if (hard_fork_version <= 5 && daemonType === "2acoin" ){
            log('info', logSystem, 'Mining pool algorithm: cryptonightlite variant 1, block version: v%d.%d', [shareBuffer[0], shareBuffer[1]]);
            hash = cryptoNightLite(convertedBlob, 1);
       	} else if (typeof config.includeHeight !== "undefined" && config.includeHeight) {
            log('info', logSystem, 'Mining pool algorithm: %s variant %d, Job Height: %d', [cnAlgorithm, cnVariant, job.height]);
            hash = cryptoNight(convertedBlob, cnVariant, job.height);
        } else {
            log('info', logSystem, 'Mining pool algorithm: %s variant %d, Hard block version: v%d.%d', [cnAlgorithm, cnVariant, shareBuffer[0], shareBuffer[1]]);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('info', logSystem,
                    'Block %s found at height %d by miner %s~%s@%s - submit result: %j',
                    [blockFastHash.substr(0, 6), job.height, miner.login, miner.workerName, miner.ip, result]
                );
