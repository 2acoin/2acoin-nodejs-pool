...
function startRpcMonitoring(rpc, module, method, params, interval) {
...
function initMonitoring() {
    ...
    for(var module in config.monitoring) {
        ...
        if ((daemonType === "bytecoin" || daemonType ==="2acoin" ) && module === "wallet") {
            if (config.wallet.api) {
                settings.rpcMethod = "/balance";
                postType = '';
            } else {
        ...
        if(settings.checkInterval) {
            startRpcMonitoring(modulesRpc[module], module, settings.rpcMethod, postType, settings.checkInterval);
        }
    }
}
