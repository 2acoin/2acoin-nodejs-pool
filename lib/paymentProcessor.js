...
var sigDigits = config.payments.denomination.toString().length;
...
function closeWallet(){
    if (config.wallet.api){
        ...
        apiInterfaces.rpcWallet("/wallet", null, function(error, result) {
            if (error){
                log('error', logSystem, 'Error with %s API request to wallet-api %j', ["walletClose", error]);
//                cback(false);
                return;
            } else {
                log('info', logSystem, 'Pool Wallet is now Closed');
            }
        });
    }
}
    ...
    async.waterfall([
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
                    ...
                    if (payoutLevel !== defaultLevel) {
                        log('info', logSystem, 'Using payout level of %s for %s (default: %s)', [ utils.getReadableCoins(minPayoutLevel[workerId], 1), workerId, utils.getReadableCoins(defaultLevel, 1) ]);
                    }
                }
        ...
        function(balances, minPayoutLevel, workerFee, callback){
            ...
            for (var worker in payments){
                ...
                // checks for appended paymentID to address
                var addr = address.split(config.poolServer.paymentId.addressSeparator);
                ...
                if (!with_payment_id) {
                   // Check for an intgrated address
                   interAddress = turtleUtil.decodeAddress(worker, '0x1fcdee')
                   if (interAddress.paymentId.length !== 0 && addresses !== 0) {
                       commandIndex++;
                       addresses = 0;
                       commandAmount = 0;
                   }
...
//                log('info', logSystem, '-- Values pushed to redis %j with fee of %s', [{address: address, amount: amount}, workerFee[worker]]);
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                ...
                if (daemonType === "bytecoin" || daemonType === "2acoin") {
                    if (config.wallet.api) {
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                    ...
                    var txHash = (daemonType === "bytecoin" || daemonType === "2acoin") ? result.transactionHash : result.tx_hash;
                ...
                for (var m in notify_miners) {
                    ...
                    log('info', logSystem, '- Payment %s - Fee %s --> Miner %s', [ utils.getReadableCoins(notify.amount, sigDigits), utils.getReadableCoins(workerFee[notify.address], sigDigits), notify.address ]);
...
//      new code to send fusion transactions for wallet
        function(callback){
            for (var i = 0; i < 10; i++){
                apiInterfaces.rpcWallet("/transactions/send/fusion/basic", {}, function(error, result) {
                    if (error){
                        if (error.error.errorCode == 36){
                            log('info', logSystem, '%s - %j', [ "fusionSend", error.error.errorMessage] );
                            callback(null);
                            return;
                        } else {
                            log('error', logSystem, 'Error with %s API request to wallet-api %j', [ "fusionSend", error ]);
                            callback(null);
                            return;
                        }
                    } else {
                        log('info', logSystem, 'Fusion Trans - %s - Sent', [ result.transactionHash]);
                    }
                });
            }
            callback(null);
