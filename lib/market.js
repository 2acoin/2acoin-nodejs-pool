...
function getExchangeMarkets(exchange, callback) {
    ...
    // CoinGecko
        ...
        apiInterfaces.jsonHttpRequest('api.coingecko.com', 443, '', function(error, response) {
            ...
            var data = {};
            for (var i in response.market_data.current_price) {
                var ticker = response.market_data.current_price[i];
            ...
            if (!error) marketRequestsCache[cacheKey] = { ts: currentTimestamp, data: data };
            callback(null, data);
        }, '/api/v3/coins/2acoin?localization=false&market_data=true&community_data=false&developer_data=false&sparkline=false');
    }
...
function getExchangePrice(exchange, base, target, callback) {
    ...
    // CoinGecko
        ...
        apiInterfaces.jsonHttpRequest('api.coingecko.com', 443, '', function(error, response) {
            if (error) log('error', logSystem, 'API request to %s has failed: %s', [exchange, error]);
            if (response.Error) log('warn', logSystem, 'CoinGecko API error: %s', [response.Error]);
