    ...
    async.waterfall([
        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in balances){
                ...
                // log('info', logSystem, '-- Fee amount stored in workerTXFee = %s', [workerTXFee]);
                // log('info', logSystem, '-- Fee amount stored in %s - workerFee   = %s', [worker, workerFee[worker]]);
                // workerFee[worker] = workerTXFee;
            };
            setTimeout(function() {
